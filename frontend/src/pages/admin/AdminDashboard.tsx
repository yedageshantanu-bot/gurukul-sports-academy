    ...
    <div className="space-y-6">
      ...
      <div className="dash-columns">
        ...
        <div className="stitch-card relative overflow-visible">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="relative flex h-2 w-2" title="Real-time Tracking">
          ...
          {/* Interactive Legend & Live Inspector Banner */}
          <div className="flex flex-wrap items-center justify-between gap-2.5 mb-4 pb-3 border-b border-white/5 text-xs">
            <div className="flex items-center gap-4 text-slate-300">
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-md bg-gradient-to-br from-emerald-400 to-teal-500 shadow-sm shadow-emerald-500/40" />
                <span className="font-medium text-slate-300">Present (Avg {todayRate}%)</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-md bg-gradient-to-br from-rose-500 to-red-600 shadow-sm shadow-rose-500/40" />
                <span className="font-medium text-slate-300">Absent / Leave</span>
              </div>
            </div>

            {/* Live Inspector Pill */}
            {hoveredIndex !== null && weeklyTrend[hoveredIndex] ? (
              <div className="flex items-center gap-2 bg-slate-800/90 border border-emerald-500/30 px-3 py-1 rounded-full text-[11px] text-emerald-300 font-semibold shadow-md animate-in fade-in zoom-in-95 duration-150">
                <Sparkles className="w-3 h-3 text-amber-400 animate-spin" style={{ animationDuration: '3s' }} />
                <span>{weeklyTrend[hoveredIndex].day}:</span>
                <span className="text-emerald-400 font-bold">{weeklyTrend[hoveredIndex].present}% Present</span>
                {weeklyTrend[hoveredIndex].absent > 0 && (
                  <>
                    <span className="text-slate-500">•</span>
                    <span className="text-rose-400 font-bold">{weeklyTrend[hoveredIndex].absent}% Absent</span>
                  </>
                )}
              </div>
            ) : (
              <span className="text-[11px] text-slate-500 hidden sm:inline-flex items-center gap-1.5">
                ...
                Hover on bars to inspect squad details
              </span>
            )}
          </div>
              ...
              return (
                <div
                  ...
                  className={`relative p-2.5 rounded-xl transition-all duration-200 cursor-pointer ${
                    isHovered
                      ? 'bg-slate-800/90 ring-1 ring-emerald-500/40 shadow-xl shadow-black/40 scale-[1.01]'
                      : 'hover:bg-slate-800/40'
                  }`}
                >
                  {/* Floating Popover Tooltip */}
                  {isHovered && (
                    <div className="absolute -top-14 left-1/2 -translate-x-1/2 z-30 pointer-events-none animate-in fade-in zoom-in-95 duration-150">
                      <div className="backdrop-blur-md bg-slate-900/95 border border-emerald-500/40 shadow-2xl rounded-xl px-3 py-2 text-xs text-white whitespace-nowrap flex items-center gap-3">
                        <div className="flex items-center gap-1.5 font-bold text-slate-200 border-r border-white/10 pr-2.5">
                        ...
                        <div className="flex items-center gap-1 text-emerald-400 font-semibold">
                          ...
                          <span className="text-[10px] text-slate-400 font-normal">({presentAthletes} Athletes)</span>
                        </div>
                        {d.absent > 0 && (
                          <div className="flex items-center gap-1 text-rose-400 font-semibold border-l border-white/10 pl-2.5">
                            <span className="w-2 h-2 rounded-full bg-rose-400 shadow-sm shadow-rose-400/50" />
                            <span>{d.absent}% Absent</span>
                            <span className="text-[10px] text-slate-400 font-normal">({absentAthletes} Athletes)</span>
                          </div>
                        )}
                      </div>
                      {/* Tooltip Downward Arrow */}
                      <div className="w-2.5 h-2.5 bg-slate-900 border-b border-r border-emerald-500/40 rotate-45 mx-auto -mt-1.5 shadow-sm" />
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-[85px_1fr_50px] items-center gap-3 text-xs">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`font-semibold transition-colors ${isHovered ? 'text-emerald-300' : 'text-slate-300'}`}>
                        {d.day}
                      </span>
                    </div>

                    {/* Progress Bar Track */}
                    <div className="h-6 bg-slate-900/90 rounded-lg p-0.5 border border-white/10 overflow-hidden flex relative shadow-inner">
                      {/* Present Segment */}
                      <div
                        style={{ width: `${animateChart ? d.present : 0}%` }}
                        className={`h-full bg-gradient-to-r from-emerald-500 via-emerald-400 to-teal-400 transition-all duration-700 ease-out relative flex items-center justify-center overflow-hidden ${
                          d.absent === 0 ? 'rounded-md' : 'rounded-l-md'
                        } ${isHovered ? 'brightness-110 shadow-[0_0_12px_rgba(16,185,129,0.5)]' : ''}`}
                      >
                        {d.present >= 25 && (
                          <span className="text-[10px] font-black text-slate-950 px-1 select-none tracking-tight">
                            {d.present}%
                          </span>
                        )}
                      </div>
                      ...
                      {d.absent > 0 && (
                        <div
                          style={{ width: `${animateChart ? d.absent : 0}%` }}
                          className={`h-full bg-gradient-to-r from-rose-500 via-rose-600 to-red-600 transition-all duration-700 ease-out relative flex items-center justify-center overflow-hidden ${
                            d.present === 0 ? 'rounded-md' : 'rounded-r-md'
                          } ${isHovered ? 'brightness-110 shadow-[0_0_12px_rgba(239,68,68,0.5)]' : ''}`}
                        >
                          {d.absent >= 25 && (
                            <span className="text-[10px] font-black text-white px-1 select-none tracking-tight">
                              {d.absent}%
                            </span>
                          )}
                        </div>
                    ...
                    <div className="flex items-center justify-end">
                      <span
                        className={`font-black text-right transition-colors ${
                          d.present === 100
