  ...
  async getActiveTemplateForEvent(eventType: WhatsAppEventType) {
    ...
    const defaultTemplates: Record<string, any> = {
      ATTENDANCE_ABSENT: {
        ...
        body: 'Dear {{parent_name}}, your ward {{student_name}} was marked ABSENT today ({{date}}) for {{batch_name}} at {{academy_name}}. Kindly contact the academy for any assistance.',
        active: true,
      },
      ATTENDANCE_PRESENT: {
      ...
      FEE_OVERDUE: {
        ...
        body: 'Dear {{parent_name}}, fee of ₹{{amount}} for {{student_name}} is now OVERDUE since {{due_date}}. Please complete the payment to avoid disruption in training.',
        active: true,
      },
      PAYMENT_SUCCESS: {
